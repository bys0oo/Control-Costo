...
document.getElementById('btn-save-expense').addEventListener('click', () => {
  ...
  if(editingExpenseId){
    state.expenses = state.expenses.map(ex => ex.id === editingExpenseId
      ? { ...ex, amount, date, category, method: selectedMethod, description, cuotas, splitCount, cardId }
      : ex
    );
    editingExpenseId = null;
  } else {
...
document.getElementById('transactions-list').addEventListener('click', (e) => {
  ...
  if(editBtn){
    const exp = state.expenses.find(ex => ex.id === editBtn.dataset.editExpense);
    if(!exp) return;
    editingExpenseId = exp.id;
    document.getElementById('expense-modal-title').textContent = 'Editar gasto';
    document.getElementById('btn-save-expense').textContent = 'Guardar cambios';
    ...
    methodsGrid.querySelectorAll('.method-btn').forEach(b => b.classList.toggle('active', b.dataset.method === selectedMethod));
    updateCardWrapVisibility();
    if(exp.cardId) document.getElementById('f-card').value = exp.cardId;

    const isCuotas = (exp.cuotas || 1) > 1;
    cuotasToggle.dataset.on = isCuotas.toString();
    document.getElementById('f-cuotas-count-wrap').classList.toggle('hidden', !isCuotas);
    document.getElementById('f-cuotas-count').value = exp.cuotas || 3;

    const isSplit = (exp.splitCount || 1) > 1;
    splitToggle.dataset.on = isSplit.toString();
    document.getElementById('f-split-count-wrap').classList.toggle('hidden', !isSplit);
    document.getElementById('f-split-count').value = exp.splitCount || 2;
